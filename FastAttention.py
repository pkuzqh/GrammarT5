...
class FastMultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, max_rel_pos=10, dropout=0.1):
    ...
    def forward(self, query, key, value, start_nodes=None, end_nodes=None, rel_q=None, rel_k=None, rel_v=None):
    ...
    def rel_attn(q, k, v, start_nodes, end_nodes, rel_q, rel_k, rel_v):
        ...
        mask_matrix = start_node_mask#torch.cat([start_node_mask, start_node_mask], dim=-2)
        ...
        #print(map_pos[-1], map_pos.size(), seq_len, batch_size, num_heads)
        #print(end_nodes)
        #print('eeeeeeeeeee')
        query_indexes = end_nodes.clone().long()#torch.cat([end_nodes, start_nodes], dim=-2)
        key_indexes = start_nodes.clone().long()#torch.cat([start_nodes, end_nodes], dim=-2)

        query_indexes = query_indexes.view(batch_size * num_heads, -1)
        key_indexes = key_indexes.view(batch_size * num_heads, -1)
        #print(query_indexes)
        #print('eeeeee', query_indexes.size(), map_pos.size())
        query_indexes += map_pos
        #print('eeeeee', query_indexes.size(), map_pos.size())
        key_indexes += map_pos

        # query and key vec of context. shape [batch_size * num_heads * (k+1) * 2, seq_len, d_k]
        #print(query_indexes, q_weights.size())
        ...
        if rel_k is not None:
            ...
            #print(q_context.size(), k_pos.size())
