...
from Multihead_Attention_Tree import MultiHeadedAttentionTree
...
class rightTransformerBlock(nn.Module):
    ...
    def __init__(self, hidden, attn_heads, feed_forward_hidden, dropout):
        ...
        super().__init__()
        self.attention1 = MultiHeadedAttentionTree(h=attn_heads, d_model=hidden)
        self.attention3 = MultiHeadedAttentionTree(h=attn_heads, d_model=hidden)
    ...
    def forward(self, x, mask, inputleft, leftmask, inputP):
        x = self.sublayer1(x, lambda _x: self.attention1.forward(_x, _x, _x, mask=mask))
        x = self.sublayer3(x, lambda _x: self.attention3.forward(_x, _x, _x, mask=inputP))
